...
func TestSubscriptionHTTPHandler_Post(t *testing.T) {
	testCases := []struct {
		...
		{"new subscription without filters", "post_body_subscription_no_filters", "", nil, nil, http.StatusCreated},
		{"new subscription with filters", "post_body_subscription_with_filters", "", nil, nil, http.StatusCreated},
		{"failed by existing subscription", "post_body_subscription_with_filters", "post_response_already_exists_error", []domain.Subscription{{Email: "person1@example.com", Pattern: domain.Pattern{Company: "SpaceX", Type: "Contractor", Salary: 1000, IsRemoteFriendly: helpers.BoolPointer(true)}}}, nil, http.StatusInternalServerError},
		{"failed by repository error", "post_body_subscription_with_filters", "post_response_repository_error", nil, errors.New("mocked repository error"), http.StatusInternalServerError},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			router := gin.Default()
			router.POST(subscriptionPath, handler.Post)
